...
if os.path.isfile(homepath+'/state/ss'+str(initialIndex)+'.out'):
    ...
    for i in range(t):        
        ...
        #MI[i] = np.real(calcMI_2pt(rho,Nqubit))
        #triMI[i] = np.real(calcTriMI(rho,Nqubit))
    ...
    data = np.loadtxt(homepath + '/negativity_' + str(fileInd) + '.out')
    negativity = np.concatenate([data, negativity])   
    ...
    data = np.loadtxt(homepath + '/SRenyi2_' + str(fileInd) + '.out')   
    secRenyiS = np.concatenate([data, secRenyiS]) 
    np.savetxt(homepath + '/SRenyi2_' + str(fileInd) + '.out', secRenyiS)
    
    data = np.loadtxt(homepath + '/MIRenyi2_' + str(fileInd) + '.out')   
    secRenyiMI = np.concatenate([data, secRenyiMI]) 
    ...
    for i in range(t):
        ...
        entropy[i], secRenyiS[i], purity[i], mutual_info[i], secRenyiMI[i] = calcS(rho, Nqubit, [n for n in range(Nqubit//2)])        
        neg[i] = calcNeg(rho, Nqubit, [n for n in range(Nqubit//2)])
    ...
    if Nqubit<13:
        fileInd = len(os.listdir(homepath))//12
    else: 
        fileInd = len(os.listdir(homepath))//10
